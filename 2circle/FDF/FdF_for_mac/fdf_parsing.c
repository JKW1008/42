...
/*   Updated: 2024/06/13 16:10:48 by kjung            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_point_list	*isometric_projection(t_prv_point prv_point, int color)
{
	...
	point2d->ypoint = (prv_point.x + prv_point.y) * \
	sin(0.523599) - prv_point.z * Z_SCALE;
...
void	make_to_array(char *line, int y, int col, t_point_list **head)
{
	char		**divided;
...
void	gnl_and_make_list(int fd, int *colum, t_point_list **head)
{
	...
	*colum = check_colum(line);
	while (line)
	{
		make_to_array(line, y, *colum, head);
		y++;
		free(line);
		line = get_next_line(fd);
	}
